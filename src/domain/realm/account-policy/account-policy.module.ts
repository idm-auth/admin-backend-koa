...
import { AccountPolicyController, AccountPolicyControllerSymbol } from '@/domain/realm/account-policy/account-policy.controller';
...
export class AccountPolicyModule extends AbstractModule {
  protected runBind(): void {
    this.container.bind(AccountPolicyRepositorySymbol).to(AccountPolicyRepository).inSingletonScope();
    this.container.bind(AccountPolicyMapperSymbol).to(AccountPolicyMapper).inSingletonScope();
    this.container.bind(AccountPolicyServiceSymbol).to(AccountPolicyService).inSingletonScope();
    this.container.bind(AccountPolicyControllerSymbol).to(AccountPolicyController).inSingletonScope();
  }
