...
import { GroupPolicyController, GroupPolicyControllerSymbol } from '@/domain/realm/group-policy/group-policy.controller';
...
export class GroupPolicyModule extends AbstractModule {
  protected runBind(): void {
    this.container.bind(GroupPolicyRepositorySymbol).to(GroupPolicyRepository).inSingletonScope();
    this.container.bind(GroupPolicyMapperSymbol).to(GroupPolicyMapper).inSingletonScope();
    this.container.bind(GroupPolicyServiceSymbol).to(GroupPolicyService).inSingletonScope();
    this.container.bind(GroupPolicyControllerSymbol).to(GroupPolicyController).inSingletonScope();
  }
