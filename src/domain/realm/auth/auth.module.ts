...
import {
  AuthController,
  AuthControllerSymbol,
} from '@/domain/realm/auth/auth.controller';
import {
  AuthService,
  AuthServiceSymbol,
...
export class AuthModule extends AbstractModule {
  protected runBind(): void {
    this.container.bind(AuthMapperSymbol).to(AuthMapper).inSingletonScope();
    this.container.bind(AuthServiceSymbol).to(AuthService).inSingletonScope();
    this.container
