...
import {
  ...
  RolePolicyRepositorySymbol,
} from '@/domain/realm/role-policy/role-policy.repository';
...
export class RolePolicyService extends AbstractCrudService<
  RolePolicySchema,
  RolePolicyDtoTypes,
  RolePolicyCreate
> {
  @inject(RolePolicyRepositorySymbol)
  protected repository!: RolePolicyRepository;

  protected buildPaginationFilter(
    filter: PaginationFilter
  ): QueryFilter<InferSchemaType<RolePolicySchema>> {
    return {};
  }

  protected buildCreateDataFromDto(
    dto: RolePolicyDtoTypes['CreateRequestDto']
  ): RolePolicyCreate {
    return {
      roleId: dto.roleId,
  ...
  protected buildUpdate(
    entity: RolePolicyEntity,
    dto: RolePolicyDtoTypes['UpdateRequestDto']
  ): RolePolicyEntity {
    return entity;
  }
}
