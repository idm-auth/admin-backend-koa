...
} from '@/domain/realm/application/application.repository';
import {
  AppEnv,
  AppEnvKey,
  AppEnvSymbol,
} from '@/infrastructure/env/appEnv.provider';
...
export class ApplicationService extends AbstractCrudService<
  ApplicationSchema,
  ApplicationDtoTypes,
  ApplicationCreate
> {
  @inject(ApplicationRepositorySymbol)
  protected repository!: ApplicationRepository;
  @inject(AppEnvSymbol) private appEnv!: AppEnv;

  protected buildCreateDataFromDto(
    dto: ApplicationDtoTypes['CreateRequestDto']
  ): ApplicationCreate {
    return {
      name: dto.name,
  ...
  protected buildUpdate(
    ...
    dto: ApplicationDtoTypes['UpdateRequestDto']
  ): ApplicationEntity {
  ...
  async upsertIdmAuthCoreAPIApplication(): Promise<ApplicationEntity> {
    ...
    this.log.info(
      { systemId, applicationId: application._id },
      'IDM Auth Core API application upserted'
    );
    return application;
  }
  ...
  async upsertIdmAuthCoreWebAdminApplication(): Promise<ApplicationEntity> {
    const systemId = this.appEnv.get(
      AppEnvKey.IDM_AUTH_CORE_WEB_ADMIN_SYSTEM_ID
    );
    ...
    this.log.info(
      { systemId, applicationId: application._id },
      'IDM Auth Core Web Admin application upserted'
    );
    return application;
  }
}
