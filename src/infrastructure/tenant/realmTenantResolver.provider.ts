...
export class RealmTenantResolver extends AbstractTenantResolver {
  ...
  async resolveTenantDbName(tenantId: string): Promise<string | null> {
    ...
    this.logger.debug({ tenantId, found: !!realm, dbName: realm?.dbName }, 'Tenant resolution complete');
    return realm?.dbName ?? null;
  }
}
